...
public class CacheTests {
    ...
    public void testLockCache() {
        ...
        Thread key1Thread = new Thread(() -> {
            testSingleKey(verbose - 1, N, latch, cache, key1);
        });

        Thread key2Thread = new Thread(() -> {
            testSingleKey(verbose - 1, N, latch, cache, key2);
        });
    ...
    private void testSingleKey(long verbose, int N, CountDownLatch latch, Cache<String, String> cache, String key) {
        Runnable r = () -> {
            latch.countDown();
            try {
                latch.await();
